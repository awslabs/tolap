...
/**
 * The `state` parameter, stored beside the verifier.
 *
 * PKCE and `state` answer different questions, which is why PKCE alone was not enough
 * here. The verifier proves the code was requested by *a* flow this browser started;
 * `state` proves it was requested by *this* flow. Without it, anyone who obtains a valid
 * authorization code of their own can force-feed it to a victim as `?code=...` and sign
 * that browser into the **attacker's** account -- a login CSRF. On this console the victim
 * then authors policy under an identity they did not choose, and the audit log records the
 * attacker's subject as the actor.
 *
 * Single-use and tab-scoped for the same reason as the verifier: it must not outlive the
 * login attempt it belongs to.
 */
const STATE_KEY = "tolap.pkce.state";
...
async function beginLogin(config: OidcConfig): Promise<void> {
  ...
  // 32 random bytes, from the same CSPRNG as the verifier. A guessable `state` is no
  // better than none, since the whole point is that an attacker cannot produce it.
  const state = base64Url(crypto.getRandomValues(new Uint8Array(32)));

  // sessionStorage, not localStorage: both values are single-use and scoped to this tab's
  // login attempt, and neither must outlive the tab.
...
async function completeLogin(
  ...
  // Cleared before any check, so a rejected attempt cannot be retried against the same
  // stored values -- and so a failure cannot leave a verifier behind for the next one.
  ...
  // Checked BEFORE the code is exchanged: the point is to never send an unsolicited code
  // to the token endpoint at all. A missing `state` is treated the same as a wrong one --
  // an authorization response that omits it did not come from a flow started here.
  if (expectedState === null || returnedState !== expectedState) {
    throw new Error(
      "This sign-in did not start in this tab. Start it again from here.",
    );
  }
...
export function SignIn({ onSignedIn, message }: SignInProps) {
  ...
  useEffect(() => {
    ...
    setExchanging(true);
    void completeLogin(config, code, params.get("state"))
      .then((token) => {
        setToken(token);
