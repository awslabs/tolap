...
describe("what an empty selection means", () => {
  it("gives the allow-list reading by default", () => {
    // Section 3: absent leaves the rule unrestricted, `[]` denies everything.
  ...
  it("gives the every-source reading for sourcePatterns", () => {
    // Section 10, the one documented exception: absent AND `[]` both mean "applies to
    // every source". The allow-list message here would tell the author the exact
    // OPPOSITE of what the policy does -- and the page's own hint above it says the right
    // thing, so they would see two contradictory statements with the wrong one attached
    // to the control they are using.
    render(
      <FieldPicker
