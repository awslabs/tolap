...
export interface FieldPickerProps {
  ...
  /**
   * What an empty selection MEANS for this list, which is not the same everywhere.
   *
   * `allowList` is the section 3 reading and the default: absent leaves the rule
   * unrestricted, `[]` denies everything.
   *
   * `everySource` is `sourcePatterns`, the one documented exception (section 10) where
   * absent AND `[]` both mean "applies to every source". Rendering the allow-list message
