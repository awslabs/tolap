...
```python
import asyncpg
from tolap_core import PolicyDefinition, PolicyMerger
...
```typescript
import { Pool } from "pg";
import { PolicyDefinition, PolicyMerger } from "@tolap/core";
...
The composite sort key on assignments enables a single `Query` to retrieve all of a user's active assignments, which the SDK's `PolicyMerger` then merges into an effective policy.
