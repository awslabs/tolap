...
`collectionPath` is **optional**, and every record-level control MUST behave consistently
when it is absent — enforcing on a discoverable collection, or refusing. None may return the
body unchanged. Unlike `db`, this category has no pushdown: an HTTP API returns what it
returns, so the post-response pipeline is the **only** line of defence rather than
defence-in-depth.

When `collectionPath` is omitted and the body is an envelope, an implementation MUST:

1. enforce on the single collection if exactly one member is a non-empty array of objects —
   the key is *discovered*, not assumed to be `results` (openFDA uses `results`,
   ClinicalTrials.gov `studies`), because recognising only one name reintroduces the bug
   below for every other API;
2. **refuse** — raising the unenforceable-result error of §4, which callers already treat as
   a denial — if two or more members qualify. Guessing is worse than refusing: enforcing a
   limit on the wrong array reads as success;
3. leave the body untouched only when no member is a record collection, since there is then
   nothing to enforce.

This is normative because it was violated. `limits.maxResults` returned the body unchanged in
all three SDKs, so `maxResults: 1` handed back every record the upstream sent, while
`allowedFields` returned an empty object and `rowFilters` withheld the body — both
fail-closed. Three controls, one missing argument, three answers, and only the unsafe one went
unnoticed: each had passing tests, and every one of those tests supplied `collectionPath`
because that is what the implementation wanted. Conformance suites SHOULD therefore test each
record-level control **with and without** the argument, and assert the controls agree with
each other; a per-control test cannot see that its siblings disagree.
...
Search, pgvector). Each renderer reports a **confidence**, because "looks right" is not the
same evidence as "observed to filter":

| Confidence | Providers | Meaning |
| --- | --- | --- |
| `verified` | Bedrock, OpenSearch, Elasticsearch, pgvector | Exercised against the live service |
| `fromGrammar` | Azure AI Search, Vertex AI Search | Written from published grammar; no service has accepted one |

A `fromGrammar` renderer MUST be read as unproven rather than probably-fine. Promoting
OpenSearch and Elasticsearch out of that state immediately exposed a fail-open that had
shipped in all three SDKs, and auditing the remaining three on documentation alone exposed a
second (see below). Neither was reachable by any unit test, because both filters were
well-formed documents that the fixtures — which we wrote — asserted exactly.
...
- A filter that matches nothing is harmless **only in a positive clause**. The post pass is
  unconditional and still drops the chunk, so an allowlist that matches nothing merely costs
  efficiency.
- **In a negated clause it is a fail-open, and this is the trap.** `NOT (something that
  matches nothing)` excludes nothing, so a `deniedTags` pushdown returns every denied chunk.
  An earlier revision of this spec said a non-matching filter "costs efficiency and nothing
  else" without qualifying it by clause polarity; two shipped defects had exactly that shape.
  A renderer MUST therefore make a negated clause match under **every** field spelling or
  mapping the deployment might use, not just the conventional one.
- The failure to avoid in the other direction is a filter that excludes a chunk the policy
  *permits*. An implementation MUST NOT approximate a rule it cannot express exactly — it
  reports the rule as unpushed and leaves it to the post pass. A pushdown that is *stricter*
  than the post pass is as much a divergence as one that is looser.

#### Two defects this asymmetry produced, recorded so they are not reintroduced

- **OpenSearch / Elasticsearch emitted `key.keyword` unconditionally.** A field mapped `text`
  carries that sub-field by dynamic-mapping convention; a field mapped `keyword` directly does
  not, so the clause matched nothing. Under `must_not` that returned every denied document,
  while the allowlist arm of the same bug returned zero — failing closed. The engine accepted
  the query both times. Renderers now match the bare field **and** the `.keyword` sub-field.
- **Vertex AI Search emitted a multi-argument `NOT ANY()`.** Discovery Engine negates only a
  single-argument `ANY()`, so `NOT key: ANY("a", "b")` is not a valid expression. It is now
  split into one negated `ANY()` per value, ANDed. The positive form keeps its values in one
  `ANY()`, because there the multi-argument spelling is a disjunction — which is what an
  allowlist means; splitting it would require a chunk to carry every tag.

pgvector was audited in the same pass, found correct as written, and has since been verified
against real PostgreSQL 17 + pgvector 0.8.1 with a real ANN search. `?|` matches a text array
element or text scalar and yields false for a numeric or boolean value, which agrees with the
post pass — §7's tag extraction collects only strings, because stringifying a non-string differs
per language and a confidentiality decision MUST NOT depend on the host language's formatting.
Extending it to match numbers was tried and reverted for making the pushdown stricter than the
post pass.

The pgvector verification is worth noting for its corpus rather than its result: embeddings were
chosen so the **denied** chunks are the nearest neighbours, so an unfiltered top-*k* search
returns only denied content. A filter that matched nothing could not hide behind a plausible
result set — which is precisely how the OpenSearch fail-open survived. A conformance suite for a
`kb` pushdown SHOULD order its corpus that way.
