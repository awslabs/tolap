...
reports success is a defect this repo has already shipped once — see
[`testing-antipatterns.md`](testing-antipatterns.md) §4, which also documents the CI step
that fails the build if these suites skip there.
...
default, against the **112** this cluster allows at its 0.5 ACU floor. That number is
`LEAST({DBInstanceClassMemory/9531392}, 5000)` from the default `aurora-postgresql16`
parameter group at 2 GiB per ACU, checked against the deployed cluster rather than
estimated.

The parameter is **`static`**, which is the counter-intuitive part: `max_connections` is
fixed when the instance starts and is *not* recomputed as Serverless v2 scales up. Scaling
to 4 ACU buys CPU and memory, not connections — so the ACU **floor**, not the ceiling, is
what bounds your pool budget. Raising either number alone trades a latency problem for connection
