...
describe("nothing reaches the load balancers except CloudFront", () => {
  it("has no 0.0.0.0/0 ingress on any security group", () => {
    // CDK's `addListener` generates "Allow from anyone on port 80" unless told otherwise.
    // Both balancers are internal so that was never internet-reachable -- but it did admit
    // anything that can route inside the VPC, including the task security group, whose
    // egress is unrestricted. A compromised container could hit the admin listener
    // directly and bypass CloudFront, the WAF, the rate limit and the access log.
    //
    // That is also what made the accepted CodeQL finding's justification incomplete:
    // security/server/README.md argues the WAF is unavoidable, which held from the
    // internet and not from inside the VPC.
    //
    // Note Trivy did not catch this: it reports unrestricted EGRESS (AWS-0104) and never
    // flagged either ALB's ingress, so this assertion is the only thing standing here.
    const open: string[] = [];

    for (const [id, group] of Object.entries(
      templates.server.findResources("AWS::EC2::SecurityGroup"),
    )) {
      const ingress = ((group.Properties as { SecurityGroupIngress?: unknown[] })
        .SecurityGroupIngress ?? []) as Array<{ CidrIp?: string }>;
      for (const rule of ingress) {
        if (rule.CidrIp === "0.0.0.0/0") open.push(id);
      }
    }
    for (const [id, rule] of Object.entries(
      templates.server.findResources("AWS::EC2::SecurityGroupIngress"),
    )) {
      if ((rule.Properties as { CidrIp?: string }).CidrIp === "0.0.0.0/0") open.push(id);
    }

    expect(open, `open ingress on: ${open.join(", ")}`).toEqual([]);
  });

  it("admits the CloudFront origin-facing prefix list on both balancers", () => {
    // Scoped to CloudFront rather than to the VPC CIDR, so the only way in is the path
    // that carries the WAF. The prefix-list id is region-specific, which is why it is
    // resolved by region rather than hardcoded once.
    const rules = Object.values(
      templates.server.findResources("AWS::EC2::SecurityGroupIngress"),
    ).map((rule) => rule.Properties as Record<string, unknown>);

    const fromCloudFront = rules.filter(
      (rule) => typeof rule.SourcePrefixListId === "string",
    );
    // One per balancer.
    expect(fromCloudFront).toHaveLength(2);
    for (const rule of fromCloudFront) {
...
describe("scaling past one task", () => {
  ...
  it("keeps the worst-case connection count inside Aurora's budget", () => {
    ...
    // is the real connection count. The cluster's ceiling is
    // `LEAST({DBInstanceClassMemory/9531392}, 5000)` at 2 GiB per ACU, so 112 at the 0.5
    // ACU floor -- and because that parameter is `static` it does NOT grow when ACUs
    // scale, making the floor the binding number.
    //
