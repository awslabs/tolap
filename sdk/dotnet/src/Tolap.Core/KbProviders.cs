...
public static class KbProviders
{
    ...
    private static KbFilterConfidence ConfidenceFor(KbProvider provider) => provider switch
    {
        ...
        // Promoted after being exercised against a real OpenSearch 2.19 and Elasticsearch 7.10
        // domain, which immediately exposed a fail-open: the renderer emitted a `.keyword`
        // sub-field the index did not have, and under `must_not` a term matching nothing
        // excludes nothing, so a denylist returned every denied document. The engine accepted
        // the query, so no unit test could have seen it.
        KbProvider.OpenSearch or KbProvider.Elasticsearch => KbFilterConfidence.Verified,
        // pgvector: exercised against real PostgreSQL 17 + pgvector 0.8.1 with a real ANN
        // search, on a corpus where the denied chunks are the NEAREST neighbours -- so a
        // filter that enforced nothing could not hide behind a plausible result set.
    ...
    /// <remarks>
    /// <para>
    /// Each clause matches the field <b>and</b> its <c>.keyword</c> sub-field, because which one
    /// exists depends on the deployment's mapping and the renderer cannot see the mapping. A
    /// field mapped <c>text</c> is analyzed -- <c>terms</c> against it matches unpredictably --
    /// and carries a <c>.keyword</c> sub-field by dynamic-mapping convention. A field mapped
    /// <c>keyword</c> directly has no such sub-field, and a <c>terms</c> clause naming one
    /// matches <b>nothing</b>.
    /// </para>
    /// <para>
    /// That asymmetry was a fail-open, found against a real OpenSearch 2.19 domain. This renderer
    /// emitted <c>classification.keyword</c> unconditionally, and its docs called a mapping
    /// mismatch "the usual harmless miss" -- reasoning that holds only for an allowlist. On a
    /// denylist the clause sits under <c>must_not</c>, so a term matching nothing <b>excludes</b>
    /// nothing: <c>deniedTags: ["secret"]</c> returned every secret document, and the engine
    /// accepted the query, so nothing looked wrong. The allowlist arm failed closed (0 hits)
    ...
    /// given document, so a positive clause still admits it and a negated clause still excludes
    /// it.
    /// </para>
    /// </remarks>
    private static object? RenderOpenSearch(KbFilterClause[] clauses)
    {
        ...
        foreach (var clause in clauses)
        {
            ...
            // Nested inside the outer must_not, De Morgan gives "excluded under EITHER
            // spelling", which is the fail-closed reading.
            var terms = new Dictionary<string, object>
            {
                ["bool"] = new Dictionary<string, object>
                {
                    ["should"] = new List<object>
                    {
                        ...
                        {
                            ["terms"] = new Dictionary<string, object> { [clause.Key] = clause.Values },
                        },
                        new Dictionary<string, object>
                        {
    ...
    /// <para>
    /// A negated clause is split into one single-argument <c>NOT ANY()</c> per value, ANDed
    /// together. Discovery Engine's negation is documented as working <i>only</i> with a
    /// single-argument <c>ANY()</c>, so <c>NOT key: ANY("a", "b")</c> -- which this renderer
    /// emitted -- is not a valid expression. It was never sent to the service, because the
    /// renderer carried <c>FromGrammar</c> and nothing had exercised it.
    /// <c>NOT ANY("a") AND NOT ANY("b")</c> is the equivalent the grammar accepts, and De Morgan
    /// makes it exactly the same predicate.
    /// </para>
    /// </summary>
    /// <remarks>
    ...
    private static object? RenderVertex(KbFilterClause[] clauses)
    {
        ...
        foreach (var clause in clauses)
        {
            ...
            if (clause.Op == KbFilterOp.In)
            {
                ...
                parts.Add($"{clause.Key}: ANY({values})");
            }
            else
            {
                foreach (var value in clause.Values)
                    parts.Add($"NOT {clause.Key}: ANY(\"{value}\")");
            }
        }
    ...
    private static object? RenderPgvector(KbFilterClause[] clauses, string column)
    {
        ...
        foreach (var clause in clauses)
        {
            ...
            // `?|` matches a text array element or a text scalar, and yields false for a numeric
            // or boolean value. That is not a gap: tag harvesting collects only strings, because
            // stringifying a non-string tag differs per language and a confidentiality decision
            // must not depend on host formatting. So {"classification": 3} carries no tag, a
            // denylist keeps it, and `?|` keeps it too. Adding a `->>`-cast arm to "also match
            // numbers" was tried and reverted: it made the pushdown STRICTER than the normative
            // post pass. Verified against real PostgreSQL 17.
