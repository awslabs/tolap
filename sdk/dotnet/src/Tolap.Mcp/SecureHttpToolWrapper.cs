...
public sealed class SecureHttpToolWrapper
{
    ...
    /// <summary>
    /// The single record-collection key in an envelope, or null if it is not unambiguous.
    /// </summary>
    /// <remarks>
    /// Used only when the caller gave no <c>collectionPath</c>. Returns a key only when the body
    /// has <b>exactly one</b> value that is a non-empty list of objects, so a body with two
    /// candidate collections is never guessed at -- guessing the wrong one would enforce the
    /// limit on the wrong array and read as success.
    /// </remarks>
    private static string? ImplicitCollectionKey(object? body)
    {
        if (body is not Dictionary<string, object?> map) return null;
        var candidates = map
            .Where(entry => entry.Value is List<object?> { Count: > 0 } items
                            && items.All(item => item is Dictionary<string, object?>))
    ...
    private static object? LimitCollection(object? body, string? collectionPath, EffectivePolicy policy)
    {
        ...
        if (collectionPath is null)
        {
            ...
            // `maxResults: 1` handed back every record the upstream sent -- a fail-open, and the
            // only one of the three record-level controls that behaved this way: the projection
            // returned an empty object and the row filter dropped the body, both fail-closed.
            // Silently disagreeing on the same missing argument is worse than any single choice,
            // so the limit now enforces on an unambiguously-identifiable collection too.
            // Ambiguous bodies throw rather than guess.
            var implicitKey = ImplicitCollectionKey(body);
            if (implicitKey is not null && body is Dictionary<string, object?> source)
            {
                var limited = new Dictionary<string, object?>(source);
                var records = (List<object?>)source[implicitKey]!;
                limited[implicitKey] = records.Count > max.Value ? records.Take(max.Value).ToList() : records;
                return limited;
            }
            if (body is Dictionary<string, object?> ambiguous
                && ambiguous.Values.Any(value => value is List<object?> { Count: > 0 } candidate
                                                 && candidate.All(item => item is Dictionary<string, object?>)))
            {
                throw new UnenforceableResultException(
                    "limits.maxResults cannot be enforced: the response body has more than one "
                    + "candidate record collection and no collectionPath was supplied. Pass "
                    + "collectionPath to name the one the limit applies to.");
            }
            return body;
        }
