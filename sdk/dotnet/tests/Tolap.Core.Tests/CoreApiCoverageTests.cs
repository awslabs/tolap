...
public class CoreApiCoverageTests
{
    ...
    [Fact]
    public void Exploit_ValidateAccess_DeniesWhenCanQueryIsFalse()
    {
        // Regression: this SDK's ValidateAccess did not check CanQuery, while Python's
        // validate_access and TypeScript's validateAccess both did -- so a policy with
        // canQuery:false had its object check PASS here alone. A fail-open on the broadest
        // permission there is, and a cross-SDK divergence for one signed policy.
        //
        // It hid because the MCP wrapper checks the gate separately before calling in. But
        // ValidateAccess is public API and a storage wrapper must call it directly to satisfy
        // connector-spec section 8 (validate the prefix BEFORE the provider call), so a
        // caller doing the right thing got no gate at all. Found by porting the S3 suite to
        // .NET; the identical omission was fixed in ValidateEndpoint earlier.
        var denied = Definition(
            "no-query",
            new ObjectRules(AllowedObjects: new[] { "patients" }),
            permissions: new PolicyPermissions(CanQuery: false, ReadOnly: true));
        var policy = PolicyMerger.Merge(new[] { denied });

        var result = EnforcementEngine.ValidateAccess("patients", policy);

        result.Allowed.Should().BeFalse("canQuery is the top-level read gate");
        result.Reason.Should().Be("query not permitted");
    }

    [Fact]
    public void ValidateAccess_AllowsWhenCanQueryIsTrue()
    {
        // The paired control: without it the test above would pass against a ValidateAccess
        // that denied everything unconditionally.
        var granted = Definition(
            "query-ok",
            new ObjectRules(AllowedObjects: new[] { "patients" }),
            permissions: new PolicyPermissions(CanQuery: true, ReadOnly: true));
        var policy = PolicyMerger.Merge(new[] { granted });
