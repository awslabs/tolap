...
public class KbMetadataFilterTests
{
    ...
    /// <summary>
    /// Two fail-opens shipped in these renderers with the same shape: a negated clause that
    /// matches nothing <b>excludes</b> nothing, so a denylist returns every denied document
    /// while the allowlist arm of the same bug fails harmlessly closed. OpenSearch emitted a
    /// <c>.keyword</c> sub-field the index did not have; Vertex emitted a multi-argument
    /// <c>NOT ANY()</c> that Discovery Engine does not accept. Both were invisible to
    /// per-provider tests asserting the document we had chosen to emit.
    /// </summary>
    [Fact]
    public void VertexNegation_IsSplitIntoSingleArgumentAny()
    {
        // `NOT key: ANY("a", "b")` is not valid per Google's documented grammar, so a two-tag
        // denylist produced a filter the service would reject or misapply.
        var rendered = (string)Render(new TagRules(DeniedTags: new[] { "secret", "restricted" }),
            KbProvider.VertexAiSearch)!;

        rendered.Should().Be(
            "NOT classification: ANY(\"restricted\") AND NOT classification: ANY(\"secret\")");
        foreach (var match in Regex.Matches(rendered, @"NOT [^:]+: ANY\(([^)]*)\)").Cast<Match>())
        {
            match.Groups[1].Value.Should().NotContain(",",
                "a multi-argument NOT ANY() is not a valid Discovery Engine expression");
        }
    }

    [Fact]
    public void Control_VertexAllowlist_KeepsValuesInOneAny()
    {
        ...
        var rendered = (string)Render(new TagRules(AllowedTags: new[] { "public", "internal" }),
            KbProvider.VertexAiSearch)!;

        rendered.Should().Be("classification: ANY(\"internal\", \"public\")");
        rendered.Should().NotContain(" AND ");
    }

    [Fact]
    public void PgvectorNegation_AdmitsAnUntaggedRow()
    {
        // `NOT (... ?| ...)` alone would drop a row whose key is absent, discarding untagged
        // chunks a denylist permits. Deliberately NOT extended to match numeric values: tag
        // harvesting collects only strings, so a `->>` arm would make the pushdown stricter
        // than the normative post pass.
        ((string)Render(new TagRules(DeniedTags: new[] { "secret" }), KbProvider.Pgvector)!)
            .Should().Contain("IS NULL");
    }

    [Fact]
    public void Azure_PlacesNotOutsideTheLambda()
    {
        // Azure rejects `any(t: not search.in(...))` and accepts `not any(t: search.in(...))`.
        var rendered = (string)Render(new TagRules(DeniedTags: new[] { "secret" }),
            KbProvider.AzureAiSearch)!;

        rendered.Should().StartWith("not classification/any(t: search.in(");
        rendered.Should().NotContain("any(t: not");
    }

    [Fact]
    public void EveryProvider_RendersADenylistAtAll()
    {
        // Vacuity guard: the tests above assert the shape of a rendered filter and would pass
        // trivially if a renderer had stopped producing one.
        foreach (var provider in Enum.GetValues<KbProvider>())
        {
            Render(new TagRules(DeniedTags: new[] { "secret" }), provider)
                .Should().NotBeNull($"{provider} rendered no filter for a denylist");
        }
    }

    private static object? Render(TagRules tagRules, KbProvider provider)
    {
        var policy = new EffectivePolicy(
            ...
            ResolvedAt: DateTimeOffset.UtcNow,
            ExpiresAt: DateTimeOffset.UtcNow.AddHours(1),
            SourceProfiles: new[] { "s" },
            Permissions: new PolicyPermissions(CanQuery: true),
            ObjectRules: new ObjectRules(TagRules: tagRules));
        return KbProviders.Render(
            KbFilter.Build(policy, new[] { "classification" }), provider).Filter;
    }
}
