...
public class SchemaFixtureConformanceTests
{
    ...
    /// <summary>
    /// The corpus as plain paths, for callers that sweep it rather than parameterise on it.
    /// </summary>
    /// <remarks>
    /// Separate from <see cref="Corpus"/> because reaching into a <c>TheoryData</c> row to get
    /// the path back out only compiled by accident: indexing a row of
    /// <c>TheoryData&lt;string&gt;</c> used to yield <c>object</c>, and now yields the row's
    /// single <c>string</c> — so <c>(string)row[0]</c> became <c>char</c>-to-<c>string</c> and
    /// stopped building. The value was never xunit's to hand back; the sweep wants the list.
    /// </remarks>
    private static List<string> CorpusPaths()
    {
