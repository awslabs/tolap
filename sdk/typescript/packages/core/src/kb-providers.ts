 ...
 * `text` is analyzed -- `terms` against it matches unpredictably -- and carries a `.keyword`
 * sub-field by dynamic-mapping convention. A field mapped `keyword` directly has no such
 * sub-field, and a `terms` clause naming one matches **nothing**.
 *
 * That asymmetry was a fail-open, found against a real OpenSearch 2.19 domain. This renderer
 * emitted `classification.keyword` unconditionally, and its docstring called a mapping mismatch
 * "the usual harmless miss" -- reasoning that holds only for an allowlist. On a denylist the
 * clause sits under `must_not`, so a term matching nothing **excludes** nothing:
 * `deniedTags: ["secret"]` returned every secret document, and the engine accepted the query, so
 * nothing looked wrong. The allowlist arm failed closed (0 hits) while the denylist arm failed
 * open, on the same mapping and the same missing sub-field.
 *
 * Matching both spellings is correct in either mapping: exactly one of the two can match a given
 * document, so a positive clause still admits it and a negated clause still excludes it.
 */
function renderOpenSearch(clauses: KbFilterClause[]): unknown {
  ...
  for (const clause of clauses) {
    // `should` + minimumShouldMatch 1 is an OR over the two possible field spellings. Nested
    // inside the outer must_not, De Morgan gives "excluded under EITHER spelling", which is the
    // fail-closed reading.
    const terms = {
      bool: {
        should: [
          { terms: { [clause.key]: clause.values } },
          { terms: { [`${clause.key}.keyword`]: clause.values } },
        ],
 ...
 * A negated clause is split into one single-argument `NOT ANY()` per value, ANDed together.
 * Discovery Engine's negation is documented as working *only* with a single-argument `ANY()`,
 * so `NOT key: ANY("a", "b")` — which this renderer emitted — is not a valid expression. It
 * was never sent to the service, because the renderer carried `fromGrammar` and nothing had
 * exercised it. `NOT ANY("a") AND NOT ANY("b")` is the equivalent the grammar does accept, and
 * De Morgan makes it exactly the same predicate.
 *
 * The positive form keeps every value in one `ANY()`: there the multi-argument spelling is the
 * documented one, and it is a disjunction, which is what an allowlist means.
 *
...
function renderVertex(clauses: KbFilterClause[]): unknown {
  ...
  for (const clause of clauses) {
    ...
    if (clause.op === KbFilterOp.In) {
      parts.push(`${clause.key}: ANY(${clause.values.map((v) => `"${v}"`).join(", ")})`);
    } else {
      for (const value of clause.values) parts.push(`NOT ${clause.key}: ANY("${value}")`);
    }
  }
...
function renderPgvector(clauses: KbFilterClause[], column: string): unknown {
  ...
  for (const clause of clauses) {
    ...
    // boolean value. That is not a gap: tag harvesting collects only strings, because
    // stringifying a non-string tag differs per language (`String(true)` vs Python's
    // `str(True)`) and a confidentiality decision must not depend on host formatting. So
    // `{"classification": 3}` carries no tag, a denylist keeps it, and `?|` keeps it too.
    // Adding a `->>`-cast arm to "also match numbers" was tried and reverted: it made the
    // pushdown STRICTER than the normative post pass. Verified against real PostgreSQL 17.
