...
| `AWS-0013` — "distribution uses an insecure minimum TLS version" | High | **A true positive, accepted — not the false positive originally claimed here.** The distribution has no `ViewerCertificate` block, which is how CloudFront selects its own managed certificate for `*.cloudfront.net`, and that certificate genuinely negotiates TLS 1.0 and 1.1. The earlier note said `MinimumProtocolVersion: TLSv1.2_2021` "is set in the CDK and applies once a custom certificate is attached"; it *is* set at `infra/lib/edge-stack.ts`, but CDK **drops it from the synthesized template** when no custom certificate is present, so it will not carry forward on its own. Whoever attaches a custom domain must set it again. Unavoidable while serving `*.cloudfront.net`, which is why it stands. |
...
| `AWS-0104` — "unrestricted egress" | Critical | Default security-group egress. The tasks need outbound access to Cognito; Secrets Manager, ECR, S3 and CloudWatch Logs already go through VPC endpoints rather than the internet. One qualification the earlier note missed: **four of the six findings are on VPC interface-endpoint security groups, which need no egress at all** — that is CDK's `allowAllOutbound` default rather than a considered trade-off, and it is the easiest of the six to actually fix. Narrowing egress to AWS service prefix lists is tracked hardening, not silently accepted. |
...
the managed groups, at 2000 requests per 5 minutes per IP (`infra/lib/edge-stack.ts`).

**A correction, because the original justification here was incomplete.** It said "the ALBs
are internal, so there is no path to these routes that skips it." That held from the
internet and not from inside the VPC: CDK's `addListener` had generated an ingress rule for
`0.0.0.0/0`, so anything that could route within the VPC — including the task security
group, whose egress is unrestricted — could reach the admin listener directly and bypass
the WAF, the rate limit and the access log. Both listeners are now `open: false` with
ingress scoped to the CloudFront origin-facing prefix list, which is what makes the claim
true as stated. `infra/test/infra.test.ts` now asserts no security group carries
`0.0.0.0/0` ingress.

Worth recording that **Trivy did not flag this.** It reports unrestricted *egress*
(`AWS-0104`, six times) and never mentioned either ALB's *ingress* on an internet-shaped
CIDR. The regression test is the only thing guarding it.
