...
function parseColumnDump(sql: string): Map<string, string[]> {
  // A Set per table, not an array.
  //
  // The accumulation below was `existing.includes(column)` before pushing, which scans a
  // linearly growing array once per row -- quadratic in the number of distinct columns and
  // reachable well inside the 2 MB body limit. Measured 104ms at 0.25 MB, 355ms at 0.5 MB,
  // 1422ms at 1 MB: the clean 4x-per-doubling signature. It holds the event loop for that
  // whole window, and this task also serves `/v1/resolve`, so the cost lands on every
  // install's policy resolution -- and a resolve that cannot complete is a denial, not a
  // delay.
  //
  // It survived the earlier sweep of this file because the regression test guarding this
  // function feeds it input that produces ZERO tables (it asserts `no tables found`), so
  // execution never reached this loop. A Set preserves insertion order, so the emitted
  ...
  return new Map([...tables].map(([table, columns]) => [table, [...columns]]));
}
