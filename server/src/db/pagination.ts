 ...
 *
 * Not defense against injection -- these are bound parameters. It is so a
 * hand-edited cursor produces a 400 that names the problem instead of a Postgres
 * cast error surfacing as a 500, which would page an operator over a bad URL.
 */
...
export function cursorInteger(value: string): string {
  if (!/^\d+$/.test(value)) {
    throw new PaginationError("cursor is not a valid pagination cursor");
  }
  return value;
