...
describe("importSqlDdl", () => {
  ...
  it("stays linear as the column count grows", () => {
    // The sixth super-linear defect in this file, and the one the earlier sweep missed.
    // `parseColumnDump` accumulated with `existing.includes(column)` before pushing, which
    // scans a linearly growing array once per row: quadratic in DISTINCT COLUMNS, measured
    // 104ms at 0.25 MB and 1422ms at 1 MB before the fix, 9ms and 17ms after.
    //
    // Why the existing linearity test could not catch it: `strips an unterminated block
    // comment without rescanning` asserts `no tables found`, so its input produces zero
    // tables and never enters the accumulation loop at all. A test can guard a function
    // and still never execute the line that matters -- which is the actual lesson here.
    //
    // A time assertion, because the defect IS the time: the old code returns the same
    // answer, just slowly.
    //
    // The size and the bound are taken from an actual mutation run rather than guessed --
    // and that mattered. At 50,000 columns the quadratic version costs 1678ms, so a 2s
    // bound there PASSES against the defect: the first version of this test was as
    // worthless as the one it was written to replace. At 100,000 (~2 MB, the body limit)
    // it costs 6422ms against 30ms fixed, so a 2s bound separates them by 3x on the
    // failing side and 60x on the passing side -- loose enough not to flake on a loaded CI
    // box, tight enough to actually fail.
    const rows: string[] = [];
    for (let i = 0; i < 100_000; i += 1) rows.push(`patients | col_${i}`);
    const dump = rows.join("\n");

    const started = performance.now();
    const manifest = importSqlDdl("db:a:b", dump);
    const elapsed = performance.now() - started;

    expect(manifest.objects[0]!.fields).toHaveLength(100_000);
    expect(elapsed).toBeLessThan(2_000);
  });

  it("keeps the first-seen order of duplicated columns", () => {
    // The Set that fixed the above must not reorder or drop anything: a manifest whose
    // field order churns between imports makes every dropdown unstable, and a dropped
    // column silently narrows what an author can pick.
    const manifest = importSqlDdl(
      "db:a:b",
      ["patients | ssn", "patients | region", "patients | ssn", "patients | region"].join(
        "\n",
      ),
    );
    // First-seen order, so `ssn` before `region` -- and the duplicates collapse.
